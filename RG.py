...
selected = option_menu(menu_title=None, options=["Introdução", "Corpos Massivos", "Raios de Luz"],
                       icons=["card-text", "box", "brightness-high"], default_index=0, orientation='horizontal',
                       )
...
if selected == "Introdução":
    ...
    st.write(
        "De acordo com a lei da gravitação universal de Newton, o campo gravitacional externo a um corpo esférico de massa  $M$  (situado na origem do sistema de coordenadas) é")
    eq1 = r'''
    $$
    ...
    st.write(eq1)
    st.write(
        "onde $G = 6,67  •  10^{-11} m^3 kg^{-1} s^{-2}$ é a constante da gravitação universal e $r$ é a distância ao corpo central.")
    st.write(
        "Uma partícula de massa $m$ sujeita a esse campo gravitacional descreve uma trajetória que é restrita a um plano e pode ser descrita pelas fuções $r(t)$ e $θ(t)$, que satisfazem")
    eq2 = r'''
    $$
    ...
    st.write(eq3)
    st.write(
        "Aqui, $L$ é o módulo do momento angular, $E$ é a energia total e a energia potencial efetiva $U_{efetiva}$ é dada por")
    eq4 = r'''
    $$
    ...
    st.write(
        "Um raio de luz, no espaço-tempo de Schwarzschild, descreve uma trajetória que também pode ser escrita na forma de um problema unidimensional efetivo. Temos:")
    eq6 = r'''
    $$
...
# def app2():
    ...
    if st.session_state['button'] == True:
        ...
        if l > np.sqrt(12):
            ...
            vlim = vmax
            rmax = 2 / umin
            st.write("O mínimo e o máximo da energia potencial efetiva são", vmin, "e", vmax,
            ...
            E = st.number_input(
                'Escolha um valor maior que o mínimo da energia potencial efetiva; calculada no passo anteior',
                step=1e-5, format="%.5f")
            ...
            if st.session_state['button11'] == True:
                if E < vmin:
                    st.write(
                        "ATENÇÃO: Você escolheu um valor de parâmetro de energia menor que o valor mínimo permitido. Escolha novamente um valor dentro do limite especificado antes de continuar.")
                else:
                    st.subheader("Insira o número de voltas completas na órbita:")
                    st.write(
                        "Para uma órbita ligada ($U_{eff,min} ≤ E < 0$), escolha também o número de órbitas que deseja traçar.")
            ...
            rmax = 80
            st.write(
                "Para esse valor do momento angular, a energia potencial efetiva não possui um mínimo ou máximo local.")
            st.write("")
...
# def app2():
    ...
    st.write(
        "Desta vez, a forma do potencial é fixa e está ilustrada abaixo. O máximo do potencial efetivo ocorre para $r = 1.5 r_g$.")
